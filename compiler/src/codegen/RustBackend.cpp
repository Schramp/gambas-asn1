...
bool RustBackend::choice_alternative_covered(const ChoiceAlternativeSpec& a) const {
    // A SEQUENCE OF alternative resolves through Generator::choice_alt_type_for
    // to a real named wrapper type (Backend::use_synthetic_seqof_member_type),
    // so it's an ordinary composite alternative here — same !mbuiltin branch
    // as any TypeRef, no is_seq_of special case needed.
