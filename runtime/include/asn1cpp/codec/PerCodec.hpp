...
private:
    ...
    static Expected<std::size_t, DecodeError> decode_size_field(
    ...
    // ---- BER-content round-trip helpers (OID, RELATIVE-OID) ---------------
    // X.691 §14: PER = unconstrained length + BER content bytes (no TLV header).
